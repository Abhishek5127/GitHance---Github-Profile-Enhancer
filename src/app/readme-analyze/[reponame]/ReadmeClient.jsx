...
function DetailChip({ children }) {
  return (
    <span className="rounded-full border border-white/10 bg-black/25 px-3 py-1.5 text-xs text-white/72">
      {children}
    </span>
  );
}

function GeneratorIntelPanel({ workspaceData, options, generationMeta }) {
  const repository = workspaceData?.repository || null;
  const activeSections =
    generationMeta?.requestedSections?.length
      ? generationMeta.requestedSections
      : README_SECTION_DEFINITIONS.filter((section) => Boolean(options?.sections?.[section.key])).map(
          (section) => section.label
        );
  const evidenceFiles = Array.isArray(generationMeta?.usedFiles) ? generationMeta.usedFiles : [];
  const repoSignals = generationMeta?.repositorySignals || {};
  const signalChips = [
    repoSignals.language || repository?.language,
    repoSignals.license || repository?.license?.name,
    repository?.private ? "Private repository" : "Public repository",
    options?.tone ? `Tone: ${options.tone}` : "",
  ].filter(Boolean);
  const topics = Array.isArray(repoSignals.topics) && repoSignals.topics.length
    ? repoSignals.topics
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[1.3fr_1fr]">
      <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
        <p className="text-xs uppercase tracking-[0.2em] text-white/45">Repository Context</p>
        <h3 className="mt-1 text-xl font-semibold text-white">What GitHance will use</h3>
        <p className="mt-1 text-sm text-white/65">
          The generator now favors repository evidence, requested section coverage, and concise GitHub-native formatting.
        </p>

        <div className="mt-4 flex flex-wrap gap-2">
          {signalChips.map((value) => (
            <DetailChip key={value}>{value}</DetailChip>
          ))}
          {topics.map((topic) => (
            <DetailChip key={topic}>#{topic}</DetailChip>
          ))}
        </div>

        <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-4">
          <p className="text-xs uppercase tracking-[0.18em] text-white/45">Evidence Files</p>
          {evidenceFiles.length ? (
            <div className="mt-3 flex flex-wrap gap-2">
              {evidenceFiles.map((file) => (
                <DetailChip key={file}>{file}</DetailChip>
              ))}
            </div>
          ) : (
            <p className="mt-2 text-sm text-white/58">
              GitHance will sample the strongest repository files when you generate the draft.
            </p>
          )}
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
        <p className="text-xs uppercase tracking-[0.2em] text-white/45">Output Format</p>
        <h3 className="mt-1 text-xl font-semibold text-white">README shape</h3>
        <div className="mt-4 space-y-3 text-sm text-white/72">
          <p>One H1, a concise opening summary, and only sections that are supported by real repository evidence.</p>
          <p>Setup and usage stay grounded in actual files, scripts, and configuration instead of generic README filler.</p>
          <p>Capabilities render best as clean bullets, short paragraphs, and code fences only when commands are verifiable.</p>
        </div>

        <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-4">
          <p className="text-xs uppercase tracking-[0.18em] text-white/45">Requested Sections</p>
          <div className="mt-3 flex flex-wrap gap-2">
            {activeSections.length ? (
              activeSections.map((section) => <DetailChip key={section}>{section}</DetailChip>)
            ) : (
              <p className="text-sm text-white/58">GitHance will choose the essentials automatically.</p>
            )}
          </div>
        </div>
      </div>
...
export default function ReadmeClient({ reponame }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const nextMeta = payload?.generationMeta || {
        requestedSections: README_SECTION_DEFINITIONS.filter((section) => Boolean(options.sections[section.key])).map(
          (section) => section.label
        ),
      ...
      setFeedback({
        tone: "success",
        message: nextMeta.usedFiles?.length
          ? `GitHance refreshed the README using ${nextMeta.usedFiles.length} repository files and your current settings.`
          : "GitHance refreshed the README using the current repository context and your settings.",
      });
    } catch (error) {
      setFeedback({
        tone: "error",
