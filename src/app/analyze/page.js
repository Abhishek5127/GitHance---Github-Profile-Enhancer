...
function matchesFilter(repo, filterId) {
  if (filterId === "missing-readme") return !repo.hasReadme;
  if (filterId === "with-readme") return repo.hasReadme;
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0f1115] p-4">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        >
          {repo.hasReadme ? "README Lab" : "Create README"}
        </Link>
        <Link
          href={securityHref}
...
function RepositoryCard({ repo }) {
  ...
  const statusClass = repo.hasReadme
    ...
    <article className="group relative overflow-hidden rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(18,22,28,0.98),rgba(11,13,15,0.98))] p-5 shadow-[0_24px_80px_rgba(0,0,0,0.32)] transition hover:-translate-y-1 hover:border-white/20">
      <div
        className={`pointer-events-none absolute right-0 top-0 h-36 w-36 rounded-full blur-3xl ${
          repo.hasReadme ? "bg-emerald-400/12" : "bg-[#ff7a1a]/14"
        }`}
      />
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className={`rounded-full border px-3 py-1 text-[11px] uppercase tracking-[0.18em] ${statusClass}`}>
            {repo.hasReadme ? "README ready" : "README missing"}
          </span>
        </div>
        ...
        <div className="mt-6 grid gap-2 sm:grid-cols-2">
          ...
          >
            {repo.hasReadme ? "Open README Lab" : "Create README"}
          </Link>
          <Link
            href={securityHref}
...
export default function AnalyzePage() {
  ...
  const requestRepositories = useCallback(async (nextPage, append = false) => {
    ...
    try {
      ...
      startTransition(() => {
        ...
        setPage(nextPage);
        setHasMore(payload.repos.length === REPOS_PER_PAGE);
  ...
  const readmeReadyCount = repositories.filter((repo) => repo.hasReadme).length;
  const missingReadmeCount = repositories.length - readmeReadyCount;
          ...
          <>
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <StatCard
                ...
                hint="Repositories where a README already exists and can be refined immediately."
                accent="emerald"
              />
              <StatCard
                label="Need README"
                value={missingReadmeCount}
                hint="Good candidates for first-time README generation through README Lab."
                accent="amber"
              />
              <StatCard
                label="Private Repos"
