...
export default function LandingFeedbackWidget() {
  ...
  const isExpanded = isHovered || isPinnedOpen || isFocusedWithin;
  ...
  const handleSubmit = (event) => {
    ...
    startTransition(async () => {
      try {
        ...
        setStatus({
          tone: "error",
          message: error instanceof Error ? error.message : "Unable to save feedback right now.",
        });
        setIsPinnedOpen(true);
      }
    });
    ...
    >
      ...
      >
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 p-4">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.3em] text-[#ffb37f]">Quick Feedback</p>
            <p className="mt-1 text-sm text-white/60">Tell us what should feel better on GitHance.</p>
          </div>

          <label className="sr-only" htmlFor="landing-feedback-message">
            Feedback message
          </label>
          <textarea
          ...
          <div className="flex items-center justify-between gap-3">
            <p
              aria-live="polite"
              className={`text-xs leading-5 ${STATUS_CLASS_MAP[status.tone] || STATUS_CLASS_MAP.info}`}
            >
