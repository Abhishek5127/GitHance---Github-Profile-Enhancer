...
type ServerProfileComparisonOptions = {
...
const userCache = new Map<string, CacheEntry<GitHubUserResponse>>();
const repositoryCache = new Map<string, CacheEntry<GitHubRepoResponse[]>>();
const readmeCache = new Map<string, CacheEntry<boolean>>();
const comparisonCache = new Map<string, CacheEntry<ProfileComparisonResult>>();

const userRequests = new Map<string, Promise<GitHubUserResponse>>();
const repositoryRequests = new Map<string, Promise<GitHubRepoResponse[]>>();
const readmeRequests = new Map<string, Promise<boolean>>();
const comparisonRequests = new Map<string, Promise<ProfileComparisonResult>>();
...
function comparisonCacheKey(leftUsername: string, rightUsername: string) {
  return `${normalizeUsername(leftUsername)}::${normalizeUsername(rightUsername)}`;
}
...
function extractGitHubErrorMessage(payload: unknown) {
...
function isRateLimitResponse(status: number, errorMessage: string) {
  if (status === 429) return true;
  if (status !== 403) return false;
  return /rate limit/i.test(errorMessage);
}

function buildRateLimitErrorMessage(hasToken: boolean) {
  if (hasToken) {
    return "GitHub API rate limit reached for the current server token. Please wait a moment and try again.";
  }

  return "GitHub API rate limit reached. Please wait a moment and try again, or configure GITHUB_TOKEN on the server to raise the limit.";
}
...
async function fetchGitHubJson<T>(
  ...
  options: ServerProfileComparisonOptions = {}
) {
  ...
  if (!response.ok) {
    ...
    const errorMessage = extractGitHubErrorMessage(payload);

    if (isRateLimitResponse(response.status, errorMessage)) {
      throw new Error(buildRateLimitErrorMessage(Boolean(token)));
    }
...
async function fetchUser(
  ...
  return loadWithCache({
    ...
    inFlight: userRequests,
    key: normalizedUsername,
    loader: () =>
      fetchGitHubJson<GitHubUserResponse>(
        `${GITHUB_API_BASE}/users/${normalizedUsername}`,
        `GitHub user "${normalizedUsername}" was not found.`,
        options
      ),
    ...
    loader: async () => {
      ...
      for (let page = 1; page <= pageCount; page += 1) {
        const pageRepositories = await fetchGitHubJson<GitHubRepoResponse[]>(
          `${GITHUB_API_BASE}/users/${normalizedUsername}/repos?per_page=100&page=${page}&sort=updated`,
          `Repositories for "${normalizedUsername}" could not be loaded.`,
          options
        );
...
async function checkRepositoryReadme(
  fullName: string,
  options: ServerProfileComparisonOptions = {}
) {
  return loadWithCache({
    cache: readmeCache,
    forceRefresh: options.forceRefresh,
    inFlight: readmeRequests,
    key: String(fullName || "").trim().toLowerCase(),
    loader: async () => {
      try {
        const token = String(options.token || "").trim();
...
async function enrichRepositories(
  rawRepositories: GitHubRepoResponse[],
  options: ServerProfileComparisonOptions = {}
) {
  const readmeAvailability = await mapWithConcurrency(
    rawRepositories,
    README_CHECK_CONCURRENCY,
    async (repository) => checkRepositoryReadme(repository.full_name, options)
  );
...
async function buildComparedProfile(
  usernameInput: string,
  options: ServerProfileComparisonOptions = {}
): Promise<ComparedProfile> {
  ...
  const repositories = await enrichRepositories(rawRepositories, options);
...
export async function compareProfiles(
  ...
  return loadWithCache({
    ...
    forceRefresh: options.forceRefresh,
    inFlight: comparisonRequests,
    key: comparisonCacheKey(leftUsername, rightUsername),
    loader: async () => {
      const [leftProfile, rightProfile] = await Promise.all([
        buildComparedProfile(leftUsername, options),
        buildComparedProfile(rightUsername, options),
      ]);
