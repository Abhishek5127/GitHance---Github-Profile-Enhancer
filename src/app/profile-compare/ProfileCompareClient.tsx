...
const PENTAGON_DIMENSIONS: ScoreDimensionKey[] = [
  "productivity",
  "impact",
  "quality",
  "diversity",
...
function PentagonGraph({ left, right }: { left: ComparedProfile; right: ComparedProfile }) {
  const size = 360;
  const center = size / 2;
  const radius = 118;
  ...
  function pointFor(score: number, index: number) {
    const angle = (-Math.PI / 2) + (index / PENTAGON_DIMENSIONS.length) * Math.PI * 2;
  ...
  const leftPolygon = polygonPoints(PENTAGON_DIMENSIONS.map((dimension) => left.scores[dimension]));
  const rightPolygon = polygonPoints(PENTAGON_DIMENSIONS.map((dimension) => right.scores[dimension]));

  return (
    <Card className="overflow-hidden p-0">
      <div className="grid gap-px bg-white/10 xl:grid-cols-[1.05fr_0.95fr]">
        <div className="bg-[linear-gradient(180deg,rgba(34,211,238,0.12),rgba(255,255,255,0.02))] p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="font-mono text-[11px] uppercase tracking-[0.22em] text-white/40">Pentagon graph</p>
              <h3 className="mt-2 text-xl font-semibold text-white">Five core dimensions, one deliberate shape.</h3>
              <p className="mt-3 max-w-xl text-sm leading-6 text-white/58">
                Productivity, impact, quality, diversity, and popularity form the pentagon. Consistency stays visible in the score lanes on the right.
              </p>
            </div>
            <Badge label="Consistency tracked separately" tone="amber" />
          </div>
          ...
          <div className="mt-6 overflow-x-auto">
            <svg viewBox={`0 0 ${size} ${size}`} className="mx-auto h-[340px] w-[340px]">
              {levels.map((level) => (
                <polygon
                  key={level}
                  points={polygonPoints(PENTAGON_DIMENSIONS.map(() => level))}
                  fill="none"
              ...
              <polygon points={leftPolygon} fill="rgba(34,211,238,0.18)" stroke="#67e8f9" strokeWidth="2.4" />
              <polygon points={rightPolygon} fill="rgba(255,122,26,0.18)" stroke="#fb923c" strokeWidth="2.4" />
            </svg>
          </div>
        </div>

        <div className="bg-[linear-gradient(180deg,rgba(255,122,26,0.12),rgba(255,255,255,0.02))] p-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {[
          ...
          },
          {
            ...
            description: "The page turns raw GitHub data into an overview, pentagon graphs, a comparison table, top repositories, and an AI-style summary.",
          },
        ].map((item) => (
        ...
        <main className="mx-auto flex w-full max-w-6xl flex-1 flex-col py-10">
          <section className="rounded-[42px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.055),rgba(255,255,255,0.03))] p-6 shadow-[0_40px_120px_rgba(0,0,0,0.45)] sm:p-8 lg:p-10">
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3 text-xs text-white/60">
              <span className="rounded-full border border-white/10 bg-black/20 px-4 py-2">Pentagon score maps</span>
          ...
          <section className="mt-8 space-y-8">
            ...
            {!isLoading && comparison && leftProfile && rightProfile ? (
              <>
                ...
                <section className="space-y-5">
                  <SectionHeading eyebrow="Section 2" title="Pentagon graphs" description="Five-sided score maps highlight the most meaningful dimensions at a glance, while consistency stays visible in the supporting score lanes." />
                  <PentagonGraph left={leftProfile} right={rightProfile} />
                </section>
