...
function shiftIsoDay(day, offsetDays = 0) {
  const epoch = isoDayToEpoch(day);
  if (!epoch) return "";

  return new Date(epoch + Number(offsetDays || 0) * DAY_MS)
    .toISOString()
    .slice(0, 10);
}

function buildIsoDayRange(startDay, endDay) {
  const startEpoch = isoDayToEpoch(startDay);
  const endEpoch = isoDayToEpoch(endDay);
  if (!startEpoch || !endEpoch || startEpoch > endEpoch) return [];

  const days = [];
  for (
    let currentDay = startDay;
    isoDayToEpoch(currentDay) <= endEpoch;
    currentDay = shiftIsoDay(currentDay, 1)
  ) {
    if (!currentDay) break;
...
function getEarliestPushEventDay(events = []) {
  ...
  (Array.isArray(events) ? events : []).forEach((event) => {
    ...
    const epoch = isoDayToEpoch(day);
    if (!epoch || epoch >= earliestEpoch) return;
...
export function mergeGithubStatsWithRecentEvents(
  stats,
  { username = "", installationId = null, events = [] } = {}
) {
  const normalizedUsername = normalizeUsername(username || stats?.github_username);
  const normalizedInstallationId = normalizeInstallationId(
    installationId ?? stats?.installation_id
  );

  const baseStats = normalizeStatsFromRecord(stats, {
    username: normalizedUsername,
    installationId: normalizedInstallationId,
  });
  ...
  const earliestPushDay = getEarliestPushEventDay(events);
  if (!earliestPushDay) {
    return baseStats;
  }

  const affectedDays = buildIsoDayRange(earliestPushDay, todayIsoDay());
  if (!affectedDays.length) {
    return baseStats;
  }

  const { snapshot } = buildSnapshotFromPushEvents({
    username: normalizedUsername,
    installationId: normalizedInstallationId,
    events,
  });

  const nextStats = normalizeStatsFromRecord(baseStats, {
    username: normalizedUsername,
    installationId: normalizedInstallationId,
  });
  const affectedDaySet = new Set(affectedDays);
  const previousAffectedTotal = affectedDays.reduce(
    (sum, day) => sum + Number(nextStats.day_counts?.[day] || 0),
    0
  );

  affectedDays.forEach((day) => {
    delete nextStats.day_counts[day];
  });

  Object.entries(snapshot.day_counts || {}).forEach(([day, count]) => {
    if (!affectedDaySet.has(day)) return;

    const commits = Number(count || 0);
    if (commits > 0) {
      nextStats.day_counts[day] = commits;
    }
  });

  const repoNames = new Set([
    ...Object.keys(nextStats.repo_day_counts || {}),
    ...Object.keys(snapshot.repo_day_counts || {}),
  ]);

  repoNames.forEach((repo) => {
    const nextRepoDayCounts = { ...(nextStats.repo_day_counts?.[repo] || {}) };

    affectedDays.forEach((day) => {
      delete nextRepoDayCounts[day];
    });

    Object.entries(snapshot.repo_day_counts?.[repo] || {}).forEach(([day, count]) => {
      if (!affectedDaySet.has(day)) return;

      const commits = Number(count || 0);
      if (commits > 0) {
        nextRepoDayCounts[day] = commits;
      }
    });

    if (Object.keys(nextRepoDayCounts).length) {
      nextStats.repo_day_counts[repo] = nextRepoDayCounts;
    } else {
      delete nextStats.repo_day_counts[repo];
    }
  });

  const nextAffectedTotal = affectedDays.reduce(
    (sum, day) => sum + Number(nextStats.day_counts?.[day] || 0),
    0
  );

  nextStats.total_commits = Math.max(
    0,
    Number(baseStats.total_commits || 0) - previousAffectedTotal + nextAffectedTotal
  );
  nextStats.last_repo =
    String(snapshot.last_repo || "").trim().toLowerCase() ||
    String(baseStats.last_repo || "").trim().toLowerCase();
  nextStats.last_updated = safeIsoString(new Date()) || String(baseStats.last_updated || "");

  pruneStatsHistory(nextStats);
  deriveStats(nextStats);

  if (!nextStats.last_repo) {
    nextStats.last_repo = getMostRecentRepo(nextStats.repo_day_counts) || "";
  }
...
function getGithubStatsForUserInMemory({
  username,
  installationId = null,
  ...
  if (!preferredRecords.length) {
    ...
    return includeHistory ? emptyStats : toPublicStats(emptyStats);
  }
  ...
  return includeHistory ? merged : toPublicStats(merged);
}
...
async function loadGithubStatsForUser({
  username,
  installationId = null,
  includeHistory = false,
}) {
  if (!isMongoConfigured()) {
    return getGithubStatsForUserInMemory({ username, installationId, includeHistory });
  }
...
export async function getGithubStatsForUser({
  username,
  installationId = null,
  includeHistory = false,
}) {
